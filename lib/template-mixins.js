...
const PARTIALS = [
  ...
  'partials/forms/text-expander',
  'partials/forms/input-time'
];
...
module.exports = function (options) {
  ...
  return function (req, res, next) {
    ...
    function textExpander(key, opts) {
      opts = opts || {};
      const fields = Object.assign({}, this.options.fields[key] || options.fields[key]);
      const sKey = getTranslationKey(fields, key, 'summary');
      const dKey = getTranslationKey(fields, key, 'details');
      return Object.assign(opts, {
        summary: t(sKey),
        details: t(dKey)
      });
    ...
    function inputTime(key, opts) {
      ...
      const hourPart = compiled['partials/forms/input-text-group'].render(inputText.call(this, key + '.parts.hour', { pattern: '[0-9]*', min: 1, max: 23, maxlength: 2 }));

      const minutePart = compiled['partials/forms/input-text-group'].render(inputText.call(this, key + '.parts.minute', { pattern: '[0-9]*', min: 1, max: 59, maxlength: 2 }));

      const children = parts.concat(hourPart, minutePart).join('\n');
    ...
    const mixins = {
      'text-expander': {
        path: 'partials/forms/text-expander',
        renderWith: textExpander
      },
      'input-time': {
        path: 'partials/forms/input-time',
        renderWith: inputTime
      },
      'input-text': {
